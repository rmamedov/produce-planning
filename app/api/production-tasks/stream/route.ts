...
export async function GET(request: Request) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      send(encodeEvent("connected", { timestamp: new Date().toISOString() }));

      const unsubscribe = productionTaskEvents.subscribe((event) => {
        send(encodeEvent("production-tasks-updated", event));
      });
